...
rownames(row_metadata) <- rownames(assay_data)  # will still throw warning, acceptable

# cols_ids <- colnames(quan_df) %>%
#   grep("hct116_paren", ., ignore.case = TRUE)
# 
# row_ids <- grep("MTHFR", quan_df$gene_name , ignore.case = T)
# 
# median(as.numeric(quan_df[row_ids, cols_ids]))
...
# Normalize directly from raw intensities (skip logTransform)
qf <- normalize(qf, i = "proteins", name = "norm_proteins", method = "center.median")

# Optional: Filter proteins with >50% missing values
...
# Extract normalized matrix for downstream
...
cols_ids <- colnames(norm_mat) %>%
  grep("hct116_paren", ., ignore.case = TRUE)

row_ids <- grep("MTHFR", rownames(norm_mat) , ignore.case = T)
